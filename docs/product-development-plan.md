...
4. TMDB 内容插件和 `en-default` 画像的最小读路径已完成并通过部署验证；管理员精确跨来源 `content_id` 映射入口、详情图片 R2 持久化、前台语言切换和插件安装门禁已交付，仍需完成英文 UI smoke、运营审批和插件配置归属迁移。当前 Web 直连 TMDB 的内置适配器只是迁移态兼容层，正式版本必须由 TMDB 插件/Sidecar 或服务承载插件独占 TMDB 密钥。
...
- 新增 `en-default` 画像，页面和 API 只读取画像上下文，不在代码中判断 Douban/TMDB；TMDB 密钥只通过 TMDB 插件/Sidecar 或承载插件的服务端运行环境注入，宿主不能重复维护一份供应商密钥。现有 Web 内置直连实现仅用于迁移，待配置控制面和服务承载/Sidecar 合同完成后下线。
