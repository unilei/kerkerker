/**
 * 元数据补齐队列过滤器单元测试（无外部网络、无 MongoDB 依赖）
 *
 * 用一个最小 Mongo 查询匹配器在内存中验证
 * shortDramaMetadataBackfillFilter 的语义：
 *  - 任一三件套字段值缺失（不存在或显式 null）且未被 missing_at_source
 *    豁免的 done 条目入队；
 *  - missing_at_source 命中任一部件即豁免该部件（数组语义 $ne）；
 *  - 有值字段不触发。
 *
 * 运行：npx tsx tests/metadata-backfill.test.ts
 */

import assert from "node:assert/strict";
import test from "node:test";

import { shortDramaMetadataBackfillFilter } from "@/lib/short-drama-db";
...
/** 迷你 doc 构造：done + enabled + 有 own_folder_fid 的基线 */
function baseDoc(extra: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    enabled: true,
    status: "done",
    own_folder_fid: "fid-1",
    ...extra,
  };
}

const filter = shortDramaMetadataBackfillFilter();
...
test("补齐队列：显式 null 等价于字段缺失（转存失败残留态）", () => {
  assert.equal(
...
test("补齐队列：missing_at_source 豁免对应部件", () => {
...
test("补齐队列：基础门槛（enabled/done/own_folder_fid）", () => {
  assert.equal(matches(baseDoc({ enabled: false }), filter), false);
  assert.equal(matches(baseDoc({ status: "transferring" }), filter), false);
  assert.equal(matches(baseDoc({ own_folder_fid: null }), filter), false);
});

test("补齐队列：空数组 missing_at_source 不豁免任何部件", () => {
  assert.equal(
