...
import { Flame, Drama, Tv, Globe, Clapperboard, Palette } from 'lucide-react';
...
export default function TvPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/douban/tv');
  ...
  const getCategoryIcon = (name: string): React.JSX.Element => {
    ...
    const iconMap: Record<string, React.JSX.Element> = {
      '热门剧集': <Flame className={`${iconClass} text-orange-500`} />,
      '国产剧': <Drama className={`${iconClass} text-red-500`} />,
      '美剧': <Globe className={`${iconClass} text-blue-500`} />,
      '日剧': <Tv className={`${iconClass} text-pink-500`} />,
      '韩剧': <Tv className={`${iconClass} text-purple-500`} />,
      '英剧': <Globe className={`${iconClass} text-indigo-500`} />,
      '综艺节目': <Clapperboard className={`${iconClass} text-yellow-500`} />,
      '日本动画': <Palette className={`${iconClass} text-green-500`} />,
    };
    return iconMap[name] || <Tv className={iconClass} />;
  };
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-lg text-gray-400">
                {loading ? '正在加载精彩内容...' : `探索 ${categories.length} 个精选分类`}
              </p>
            </div>
          </div>
        </div>
